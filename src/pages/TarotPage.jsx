...
function TarotPage() {
  ...
  const { user } = useAuth();
  const {
    mutate: generateReading,
    isPending,
    error: mutationError,
    reset,
  } = useGenerateReading();
  const [videoAtualIndex, setVideoAtualIndex] = useState(() =>
    Math.floor(Math.random() * listaDeVideos.length)
  );
  ...
  const handleGenerateReading = useCallback(
    (spreadType, questionData) => {
      setFormError(null);
      reset();

      if (!user && visitorHasRead) {
        setFormError(
          'Você já realizou sua leitura de teste gratuita. Cadastre-se ou faça login para leituras ilimitadas!'
        );
        return;
      }
      ...
      generateReading(
        { question: questionData, user: user || null, spreadType },
        {
          onSuccess: (data) => {
    ...
    [generateReading, navigate, reset, user, visitorHasRead]
  );
  ...
  const defaultForm = (
    <div className={styles.formContainer}>
      ...
      <div className={styles.buttonGroup}>
        <button
          onClick={() => setSelectedSpread('celticCross')}
          className={`${styles.submitButton} ${
            selectedSpread === 'celticCross' ? styles.activeButton : ''
          }`}
        >
          Cruz Celta
        </button>
        <button
          onClick={() => setSelectedSpread('threeCards')}
          className={`${styles.submitButton} ${
            selectedSpread === 'threeCards' ? styles.activeButton : ''
          }`}
        >
          3 Cartas
        </button>
        <button
          onClick={() => setSelectedSpread('templeOfAphrodite')}
          className={`${styles.submitButton} ${
            selectedSpread === 'templeOfAphrodite' ? styles.activeButton : ''
          }`}
        >
          Templo de Afrodite
        </button>
        <button
          onClick={() => setFormType('pathChoice')}
          className={styles.submitButton}
        >
          Escolha de Caminho
        </button>
          ...
          {suggestedQuestions[selectedSpread]?.length > 0 && (
            <div className={styles.suggestionsContainer}>
              <h4 className={styles.suggestionTitle}>
                Não sabe o que perguntar? Tente uma destas:
              </h4>
  ...
  const pathChoiceForm = (
    <div className={styles.formContainer}>
      <p className={styles.subtitle}>
        Descreva os dois caminhos que você está considerando.
      </p>
      <input
        type="text"
        className={styles.pathInput}
      ...
      <div className={styles.buttonGroup}>
        <button
          onClick={handlePathChoiceReading}
          disabled={isPending || (!user && visitorHasRead)}
          className={styles.mainSubmitButton}
        >
      ...
      <div className={styles.conteudoCentralizado}>
        ...
        {mutationError && (
          <p className={styles.errorMessage}>
            Falha ao iniciar leitura: {mutationError.message}
          </p>
        )}

        {!user && visitorHasRead && formType !== 'pathChoice' && !selectedSpread && (
          <p className={styles.limitMessage}>
            Você já utilizou sua leitura de teste gratuita.{' '}
            <Link to="/cadastro">Cadastre-se</Link> ou <Link to="/login">faça login</Link>{' '}
            para leituras ilimitadas.
          </p>
        )}
      </div>
    </div>
