...
// Função getQuestionText agora tenta fazer parse de JSON
const getQuestionText = (question, spreadType) => {
  ...
  if (typeof question === 'string') {
    try {
      ...
      console.warn("Não foi possível fazer parse da string 'question' como JSON:", error);
      return question;
    }
  }
...
function PastReadingPage() {
  ...
  const chatContext = useMemo(() => {
    ...
    if (currentReading.interpretation_data?.interpretationType === 'structured') {
      mainInterpretationText =
        currentReading.interpretation_data.data?.interpretacao?.resumo ||
    ...
    const cardsText = currentReading.cards_data
      ? currentReading.cards_data
          .map(
            (card, i) =>
              `${i + 1}. ${card.nome} ${card.invertida ? '(Inv)' : ''}`
          )
          .join(', ')
  ...
  if (isError) {
    return (
      <main className="content_wrapper">
        <p>Erro ao carregar leitura: {error?.message || 'Erro desconhecido'}</p>
      </main>
    );
  }
    ...
    if (readingId?.startsWith('temp-') && !temporaryReadingData) {
      return (
        ...
        >
          ...
          <p>
            Para salvar suas leituras, <Link to="/cadastro">crie uma conta</Link> ou{' '}
            <Link to="/login">faça login</Link>.
          </p>
          <Link to="/tarot" className={styles.backLink}>
            Fazer Nova Leitura Teste
          </Link>
        </main>
      );
    }
    ...
    switch (currentReading.spread_type) {
      case 'threeCards':
        return cards.length >= 3 ? (
          <ThreeCardLayout cards={cards} basePath={basePath} />
        ) : (
          <p>Layout indisponível (dados incompletos).</p>
        );
      case 'templeOfAphrodite':
        return cards.length >= 7 ? (
          <TempleOfAphroditeLayout cards={cards} basePath={basePath} />
        ) : (
          <p>Layout indisponível (dados incompletos).</p>
        );
      case 'pathChoice':
        return cards.length >= 8 ? (
          <PathChoiceLayout cards={cards} basePath={basePath} />
        ) : (
          <p>Layout indisponível (dados incompletos).</p>
        );
      case 'celticCross':
      default:
        ...
        return cards.length >= 3 ? (
          <ThreeCardLayout cards={cards.slice(0, 3)} basePath={basePath} />
        ) : (
          <p>Layout indisponível (dados incompletos).</p>
        );
    }
  ...
  const authorUsername =
    currentReading.profiles?.username || (isOwner ? user.profile?.username : 'desconhecido');
    ...
    <div className="content_wrapper">
      <div className={styles.container}>
        ...
        <h2 className={styles.question}>
          Revisitando sua pergunta: "
          {getQuestionText(currentReading.question, currentReading.spread_type)}"
        </h2>

        <div className={styles.resultsContainer}>
          <div className={styles.cardsSection}>{renderCardLayout()}</div>
        ...
        {!isTemporary && (
          <div className={styles.communitySection}>
            <ReadingInteractionBar reading={currentReading} user={user} isOwner={isOwner} />

            {currentReading.is_public && (
              <div className={styles.publicContentGrid}>
                <div className={styles.reflectionColumn}>
                  <AuthorPost text={currentReading.shared_title} username={authorUsername} />
                </div>
        ...
        {!isTemporary && isOwner && (
          <div className={styles.privateSection}>
            ...
            <p className={styles.privateNotice}>
              Somente você pode ver este chat com a I.A.
            </p>
