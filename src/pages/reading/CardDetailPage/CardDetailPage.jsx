...
import {
  POSICOES_CRUZ_CELTA,
  POSICOES_TRES_CARTAS,
  POSICOES_TEMPLO_AFRODITE,
  POSICOES_ESCOLHA_CAMINHO,
...
function CardDetailPage() {
  ...
  const {
    ...
    isError,
    error,
  } = useSingleReading(readingId);

  const {
    cardDataFromReading,
    cardDetailsFromLibrary,
    positionName,
    backLinkUrl,
    interpretation,
  } = useMemo(() => {
    ...
    const positionIndex = Number.parseInt(position, 10) - 1;
    if (Number.isNaN(positionIndex) || positionIndex < 0) return {};

    const cardFromReading = readingData.cards_data[positionIndex];
    if (!cardFromReading) return {};

    const cardFromLibrary = baralhoDetalhado.find((card) => card.id === cardFromReading.id);

    const positionArray = positionMap[readingData.spread_type] || [];
    const realPositionIndex =
      readingData.spread_type === 'pathChoice' ? positionIndex % 4 : positionIndex;
    ...
    const specificInterpretation =
      readingData.interpretation_data?.data?.interpretacao?.analise_cartas?.[positionIndex]
        ?.texto ||
  ...
  if (isError) {
    return (
      <div className="content_wrapper">
        <p>Erro ao carregar leitura: {error.message}</p>
      </div>
    );
  }
  ...
  return (
    <div className={`content_wrapper ${styles.pageContainer}`}>
      <Link to={backLinkUrl} className={styles.backLink}>
        ← Voltar para a tiragem completa
      </Link>
      ...
      <div className={styles.layoutGrid}>
        ...
        <div className={styles.leftColumn}>
          <img
            ...
            className={`${styles.cardImage} ${
              cardDataFromReading.invertida ? styles.inverted : ''
            }`}
          />
        </div>
        ...
        <div className={styles.rightColumn}>
          ...
          <h1 className={styles.cardName}>
            {cardDetailsFromLibrary.nome}{' '}
            {cardDataFromReading.invertida ? '(Invertida)' : ''}
          </h1>
          <p className={styles.cardType}>
            {cardDetailsFromLibrary.tipo}
            {cardDetailsFromLibrary.naipe ? ` - ${cardDetailsFromLibrary.naipe}` : ''}
          </p>
          ...
          <div className={styles.keywordsGrid}>
            <div className={styles.keywordSection}>
              ...
              <ul>
                {cardDetailsFromLibrary.palavras_chave.direito.map((keyword, index) => (
            ...
            <div className={styles.keywordSection}>
              ...
              <ul>
                {cardDetailsFromLibrary.palavras_chave.invertido.map((keyword, index) => (
                  <li key={index}>{keyword}</li>
                ))}
              </ul>
            </div>
