  ...
  const renderForm = () => (
    <form onSubmit={handleSubmit} className={styles.numerologyForm}>
      <label htmlFor="birthDate" className={styles.dateLabel}>
        {isSuccessResetting
          ? 'Insira sua data correta para recalcular:'
  ...
  const renderResult = (resultData) => {
    ...
    return (
      <div className={styles.resultsContainer}>
        <div className={styles.lifePathNumberDisplay}>{resultData?.life_path_number ?? '?'}</div>
        <div className={styles.resultContent}>
          ...
          <div className={`${styles.resultCard} ${styles.birthdayCard}`}>
            <h3 className={styles.cardTitle}>
              Número do Aniversário: {resultData?.birthday_number ?? 'N/A'} (Dia {dayOfMonth})
            </h3>
            <div className={styles.cardSubSection}>
        ...
        <div className={styles.resultActions}>
          <button
            ...
            disabled={isResetting || isCalculating}
          >
    ...
    <div className={`content_wrapper ${styles.pageContainer}`}>
      <div className={styles.content}>
        ...
        {!isLoadingReading &&
          !isCalculating &&
          !isResetting &&
          (errorLoadingReading || errorCalculating || errorResetting) && (
            <div className={styles.errorContainer}>
              <p className={styles.errorMessage}>
                Erro:{' '}
                {(errorLoadingReading || errorCalculating || errorResetting)?.message ||
                  'Ocorreu um erro.'}
              </p>
