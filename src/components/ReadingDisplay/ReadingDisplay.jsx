...
const StructuredInterpretation = ({ data }) => {
  ...
  if (!interpretacao) {
    console.error(
      'Erro de Renderização: Dados estruturados inválidos para StructuredInterpretation.',
      data
    );
...
const SimpleInterpretation = ({ text }) => {
  const renderTextWithHighlights = (paragraphText) => {
    ...
    return paragraphText.split(regex).map((part, index) =>
      index % 2 === 1 ? (
        <span key={index} className={styles.cardNameHighlight}>
          {part}
        </span>
      ) : (
        part
...
function ReadingDisplay({ readingData }) {
  ...
  if (interpretation_data?.interpretationType === 'structured') {
    ...
    if (!structuredData) {
      console.error(
        "Erro de Renderização: 'interpretationType' é 'structured', mas 'data' está em falta.",
        interpretation_data
      );
      return (
        <div className={styles.container}>
          <p>Erro ao carregar dados da interpretação.</p>
        </div>
      );
    }
    ...
    switch (spread_type) {
      case 'templeOfAphrodite':
        return (
          <div className={styles.container}>
            <TempleOfAphroditeInterpretation data={structuredData} />
          </div>
        );
      case 'pathChoice':
        return (
          <div className={styles.container}>
            <PathChoiceInterpretation data={structuredData} />
          </div>
        );
      case 'celticCross':
        return (
          <div className={styles.container}>
            <CelticCrossInterpretation data={structuredData} />
          </div>
        );
      case 'threeCards':
      default:
        return (
          <div className={styles.container}>
            <StructuredInterpretation data={structuredData} />
          </div>
        );
    }
  }
  ...
  return (
    <div className={styles.container}>
      <SimpleInterpretation
        text={main_interpretation || 'Nenhuma interpretação disponível.'}
      />
    </div>
  );
}
